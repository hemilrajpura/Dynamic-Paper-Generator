<?php
    ...
    $sem = $_SESSION["semes"];
    $degreeCode = $_SESSION["degcode"];
    $subjectCode = $_SESSION["subcode"];
    $chapterNumber = $_SESSION["chap"];
    $nul = '';    //database call
    ...
    if(isset($_POST["btn-insert"]))
    {
        ...
        if ($question == '') {
            $nul = 'Question Description Missing';
                ...
                $query = "UPDATE tblquestion SET modifiedDate = '$createdDate' ,questionMarks = $marks , questionDesc = '$question' WHERE sysId = $sysid";
                if(mysqli_query($conn,$query))
                {
                    echo "<script>alert('Question updated without Image')</script>";
                }
                else{
                    echo 'error';
            ...
            {
                $query = "UPDATE tblquestion SET questionMarks = $marks, questionDesc = '$question' ,questionImage = 'file' WHERE sysId = $sysid";
                if(mysqli_query($conn,$query))
                {
                    echo "<script>alert('Question update with Image')</script>";
                }
                else{
                    echo 'error';
                    echo mysqli_error($conn);
...
<head>
    ...
    <title>pGen- Update Questions</title>
</head>
...
<nav class="navbar navbar-expand-lg navbar navbar-dark bg-dark">
    ...
    <div style="height: 50px">
        <p class="text-info bg-dark"><?php echo "sysid: ".$sysid. " -- Sem: ".$sem." -- Degree: ".$degreeCode." -- Subject: ".$subjectCode." -- Chapter: ".$chapterNumber ?></p>
    </div>
    <form method="POST" enctype="multipart/form-data">
        <div class="container-md col-md-6 border border-primary rounded-3"
            ...
            <input type="submit" name="btn-insert" value="Update" class="btn btn-primary" style="margin-top:30px;" id="btn-insert" />
        </div>
    </form>
